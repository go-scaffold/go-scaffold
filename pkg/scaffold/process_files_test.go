...
func Test_ProcessFiles_Fail_ShouldReturnErrorIfFileProviderReturnsError(t *testing.T) {
	...
	processor := scaffold.NewFileProcessor(validData, outDir, &scaffold.TemplateHelper{}, false)
	err := scaffold.ProcessFiles(provider, processor)
...
func Test_ProcessFiles_Success_ShouldCreateOutputFiles_DirWithoutSuffixOnlyTemplates(t *testing.T) {
	...
	processor := scaffold.NewFileProcessor(validData, outDir, &scaffold.TemplateHelper{}, true)
	err := scaffold.ProcessFiles(provider, processor)
